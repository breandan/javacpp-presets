...
case $PLATFORM in
    ...
    linux-x86)
        ...
        ATLAS=None CC="gcc -m32" "$CPYTHON_PATH/bin/python3.8" setup.py build -j $MAKEJ build_ext -I$CPYTHON_PATH/include/ -L$CPYTHON_PATH/lib/ install --prefix $INSTALL_PATH
        strip $(find ../ -iname *.so)
        ;;
    linux-x86_64)
        # setup.py won't pick up the right libgfortran.so without this
        ...
        mkdir -p "$PYTHONPATH"
        chmod +x "$CPYTHON_PATH/bin/python3.8"
        ATLAS=None CC="gcc -m64" "$CPYTHON_PATH/bin/python3.8" setup.py build -j $MAKEJ build_ext -I$CPYTHON_PATH/include/ -L$CPYTHON_PATH/lib/ install --prefix $INSTALL_PATH
        strip $(find ../ -iname *.so)
        ;;
    macosx-*)
        export LD_LIBRARY_PATH="$OPENBLAS_PATH/lib/:$CPYTHON_PATH/lib/"
        ...
        mkdir -p "$PYTHONPATH"
        chmod +x "$CPYTHON_PATH/bin/python3.8"
        ATLAS=None "$CPYTHON_PATH/bin/python3.8" setup.py build -j $MAKEJ build_ext -I$CPYTHON_PATH/include/ -L$CPYTHON_PATH/lib/ install --prefix $INSTALL_PATH
